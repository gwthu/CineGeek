import 'package:cinegeek/helper/utility.dart';
...
void main() {
  group("Check Date time", () {
    test('Check post time', () {
      ...
      var sec = getChatTime(date.add(const Duration(seconds: -8)).toString());
      expect(sec, "8 s");

      var min = getChatTime(date.add(const Duration(minutes: -8)).toString());
      expect(min, "8 m");

      var hour = getChatTime(date.add(const Duration(hours: -8)).toString());
      expect(hour, "8 h");

      var yesterday =
          getChatTime(date.add(const Duration(days: -1)).toString());
